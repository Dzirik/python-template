...
- *marimo*
    - Folder for Marimo reactive notebooks. Marimo is a next-generation Python notebook with reactive execution.
    - Structure mirrors the `notebooks/` folder for consistency.
    - Contains:
        - *documentation/* - Documentation notebooks demonstrating features and modules
        - *final/* - Production-ready notebooks for deliverables and reports
        - *raw/* - Work in progress, exploration, and prototyping notebooks
        - *template/* - Template notebooks for starting new projects
    - **Key difference from Jupyter:** Marimo notebooks are stored as pure Python `.py` files by default (no Jupytext needed).
- *src*
...
Marimo Notebooks:
 - make marimo: Starts Marimo editor in the marimo folder.
 - make marimo-app notebook=<filename.py>: Runs a Marimo notebook as an interactive app.
 - make marimo-new notebook=<filename.py>: Creates a new Marimo notebook.
 - make marimo-convert notebook=<path>: Converts a Jupyter notebook to Marimo format.
...
### marimo
@STARTS MARIMO EDITOR
Launches Marimo editor in the marimo folder.
The editor will automatically open in your default web browser.
Press Ctrl+C in the terminal to stop the server.

Usage: make marimo

Features:
 - Reactive notebook environment (cells auto-update when dependencies change)
 - Git-friendly (notebooks stored as pure Python .py files)
 - No hidden state issues (deterministic execution)
 - Built-in UI components for interactive data exploration
 - Native support for Plotly charts with selection events (mo.ui.plotly)

Marimo folder structure:
 - marimo/raw/ - Work in progress notebooks
 - marimo/final/ - Finalized notebooks
 - marimo/documentation/ - Documentation notebooks
 - marimo/template/ - Notebook templates
@

### marimo-app
@RUNS MARIMO NOTEBOOK AS INTERACTIVE APP
Runs a specific Marimo notebook as a read-only interactive application.
The app will automatically open in your default web browser.
Press Ctrl+C in the terminal to stop the server.

Usage: make marimo-app notebook=<filename.py>

Examples:
 - make marimo-app notebook=raw/my_notebook.py
 - make marimo-app notebook=documentation/demo.py
 - make marimo-app notebook=final/analysis.py

App mode features:
 - Read-only view (code cells hidden by default)
 - Interactive UI elements still work
 - Perfect for sharing results with non-technical users
 - Can be deployed as a web application

Note: The notebook path is relative to the marimo folder.
@

### marimo-new
@CREATES NEW MARIMO NOTEBOOK
Creates a new Marimo notebook and opens it in the editor.
The editor will automatically open in your default web browser.
Press Ctrl+C in the terminal to stop the server.

Usage: make marimo-new notebook=<filename.py>

Examples:
 - make marimo-new notebook=raw/my_new_notebook.py
 - make marimo-new notebook=documentation/new_doc.py

Note: The notebook path is relative to the marimo folder.
If the file already exists, an error will be shown.
@
...
<a name="marimo-notebooks"></a>
# Marimo Notebooks
[ToC](#table-of-content)

[Marimo](https://marimo.io/) is a next-generation Python notebook that provides a reactive, reproducible, and Git-friendly alternative to Jupyter notebooks. Unlike traditional notebooks, Marimo automatically tracks dependencies between cells and re-executes them when their inputs change, eliminating hidden state issues.

This repository supports both Jupyter and Marimo notebooks, allowing you to choose the best tool for each task.

<a name="why-marimo"></a>
## Why Marimo
[ToC](#table-of-content)

Marimo addresses several pain points commonly experienced with traditional Jupyter notebooks:

**Reactivity:**
- ✅ Cells automatically re-run when their dependencies change
- ✅ No more "run all cells" to ensure consistency
- ✅ DAG-based execution order (not linear)

**Reproducibility:**
- ✅ No hidden state - notebook state always matches code
- ✅ Deterministic execution order
- ✅ Cannot reference variables before they are defined

**Version Control:**
- ✅ Stored as pure Python `.py` files (no JSON metadata)
- ✅ Clean diffs in Git
- ✅ Easy code review and collaboration
- ✅ No Jupytext configuration needed

**Interactive UI:**
- ✅ Built-in UI components (`mo.ui.slider`, `mo.ui.dropdown`, etc.)
- ✅ Native Plotly integration with selection events (`mo.ui.plotly`)
- ✅ Two-way binding between UI and Python code

**When to Use Marimo vs Jupyter:**

| Use Case | Recommended Tool |
|----------|------------------|
| Interactive data exploration with reactive updates | **Marimo** |
| Building interactive dashboards and apps | **Marimo** |
| Plotly charts with selection/click events | **Marimo** |
| Legacy notebooks or existing workflows | **Jupyter** |
| Parameterized batch execution (Papermill) | **Jupyter** |
| Collaboration with non-Marimo users | **Jupyter** |

<a name="marimo-folder-structure"></a>
## Marimo Folder Structure
[ToC](#table-of-content)
...
```bash
# Open Marimo editor (browse all notebooks in marimo folder)
make marimo

# Create a new notebook
make marimo-new notebook=raw/my_analysis.py
...
Marimo notebooks can be deployed as interactive web applications:

```bash
# Run in app mode (read-only, interactive UI still works)
make marimo-app notebook=final/dashboard.py

# Or directly with marimo CLI
marimo run marimo/final/dashboard.py --host 0.0.0.0 --port 8080
```

<a name="interactive-visualizations-with-plotly"></a>
## Interactive Visualizations with Plotly
[ToC](#table-of-content)

One of Marimo's most powerful features is native support for interactive Plotly charts with bi-directional communication. This enables selection-based filtering, click events, and dynamic data exploration.
...
def __(chart, mo):
    ...
    selected = chart.value  # Returns selected data as dict

    if selected and "points" in selected:
        selected_indices = [p["pointIndex"] for p in selected["points"]]
        mo.md(f"**Selected indices:** {selected_indices}")
    else:
        mo.md("*Click on bars to select data points*")
    return selected,
```
...
@app.cell
def __(mo, px, df):
    # Create bar chart with selection enabled
...
def __(chart, df, mo):
    ...
    if selected and selected.get("points"):
        selected_products = [p["x"] for p in selected["points"]]
        filtered_df = df[df["product"].isin(selected_products)]
        mo.md(f"### Filtered Data ({len(filtered_df)} rows)")
        mo.ui.table(filtered_df)
    else:
        mo.md("*Select bars to filter the data table*")
```

**Tips for Interactive Visualizations:**

1. **Enable selection mode:** Use `fig.update_layout(dragmode="select")` for box selection
2. **Multiple selection:** Hold Shift while selecting to add to selection
3. **Clear selection:** Double-click on the chart background
4. **Combine with UI elements:** Use sliders, dropdowns, and other `mo.ui` components alongside charts

**Visualization Library Integration:**

The repository's visualization classes in `src/visualisations/` work seamlessly with Marimo:

```python
from src.visualisations.plotly_histogram import PlotlyHistogram
import marimo as mo

# Create histogram using repository visualization class
histogram = PlotlyHistogram()
fig = histogram.plot(data=my_data, plot_title="Distribution", x_title="Values")

# Wrap for Marimo interactivity
chart = mo.ui.plotly(fig)
chart
```
