...
> Scheduler, see [`docs/PERSISTENT_RUN.md`](PERSISTENT_RUN.md).
...
- **Add arguments** — The `-u` flag disables output buffering so log messages are written
  ...
  HOCON configuration file (e.g. `watchdog_cmd_02` loads
  `configurations/watchdog_cmd_02.conf`) and determines which heartbeat directory and
...
- The configuration file exists at `configurations\<your_config_name>.conf`.
- The `--config-name` value in the **Add arguments** field exactly matches the filename
  (without the `.conf` extension), including capitalisation.
