...
- **Add arguments** — The `-u` flag disables output buffering so log messages appear
  ...
  selects which TOML configuration file to load (e.g. `watchdog_cmd_01` loads
  `configurations/watchdogs/watchdog_cmd_01.toml`). The script exits immediately with an error if
